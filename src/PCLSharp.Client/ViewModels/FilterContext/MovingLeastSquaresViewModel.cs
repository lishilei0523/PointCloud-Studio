...
namespace PCLSharp.Client.ViewModels.FilterContext
{
    ...
    /// 移动最小二乘法重采样视图模型
    /// </summary>
    public class MovingLeastSquaresViewModel : PreviewViewModel
    {
        #region # 字段及构造器
        ...
        /// 依赖注入构造器
        /// </summary>
        public MovingLeastSquaresViewModel(ICloudCommon cloudCommon, ICloudFilters cloudFilters)
            : base(cloudCommon)
        {
        ...
        public async void Apply()
        {
            ...
            Point3F[] filterdPoints = await Task.Run(() => this._cloudFilters.ApplyMovingLeastSquares(points, this.Radius!.Value));
