...
namespace PCLSharp.Client.ViewModels.SegmentationContext
{
    ...
    /// 分割球体视图模型
    /// </summary>
    public class SphereViewModel : PreviewViewModel
    {
        #region # 字段及构造器
        ...
        /// 依赖注入构造器
        /// </summary>
        public SphereViewModel(ICloudCommon cloudCommon, ICloudSegmentations cloudSegmentations)
            : base(cloudCommon)
        {
        ...
        public async void Apply()
        {
            ...
            Point3F[] segmentedPoints = await Task.Run(() => this._cloudSegmentations.SegmentSphere(points, this.OptimizeCoefficients!.Value, this.Probability!.Value, this.DistanceThreshold!.Value, this.MinRadius!.Value, this.MaxRadius!.Value, this.MaxIterationsCount!.Value, out Point3F center, out float radius));
